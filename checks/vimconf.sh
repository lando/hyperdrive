...
if [ ! -z "$HYPERDRIVE_VERSION" ]; then
  MIN_VIMCONF_VERSION=$HYPERDRIVE_VERSION
else
  MIN_VIMCONF_VERSION=$(git describe --tags --always --abbrev=0)
fi
...
check_vimconf() {
  ...
  if [[ $VIMCONF_INSTALLED == "true" ]]; then
    ...
    semverGTE $VIMCONF_VERSION $MIN_VIMCONF_VERSION || VIMCONF_ACTION=$(status_warn "upgrade vimconf")
